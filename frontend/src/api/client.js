...
export const api = {
  ...
  salesReport: (period) => request(`/sales/reports/${period}`),
  payroll: () => request("/payroll"),
  createPayroll: (body) => request("/payroll", { method: "POST", body: JSON.stringify(body) }),
  expenses: () => request("/finance/expenses"),
  createExpense: (body) => request("/finance/expenses", { method: "POST", body: JSON.stringify(body) }),
  profitLoss: () => request("/finance/profit-loss"),
  rawMaterials: () => request("/production/raw-materials"),
  createRawMaterial: (body) => request("/production/raw-materials", { method: "POST", body: JSON.stringify(body) }),
  productionBatches: () => request("/production/batches"),
  createProductionBatch: (body) => request("/production/batches", { method: "POST", body: JSON.stringify(body) }),
};
